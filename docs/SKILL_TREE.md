...
| Cortex-state driven generation | CompLing | Expert | **DONE — T13.1+T13.2+T13.3+T13.6+T13.7 2026-04-14** — `LanguageCortex.generate()` is the T13 brain-driven emission loop. Reads live `cortex.getSemanticReadout` as the target vector per emission, scores dictionary via `cosSim · (1 + arousal·(valenceMatch−0.5)) · recencyMul`, softmax-samples top-5 at `temperature = 0.25 + (1−coherence)·0.35`, injects emitted word back into cortex at `strength=0.35` (efference copy), ticks LIF 3 steps between emissions. Natural stopping via drift quiescence (`<0.08`), grammatical terminability, or hard cap. **T13.2 parse-tree injection** (`brain.injectParseTree(text)`) routes content → cortex, intent → BG, self-ref → hippocampus before the 20-tick settle. **T13.1 persona Hebbian** trains cortex recurrent weights on persona corpus at boot. **T13.7** deleted all slot-prior machinery — `_slotCentroid`, `_slotDelta`, `_slotTypeSignature`, `_contextVector`, attractor vectors, `_subjectStarters`, `_generateSlotPrior` fallback, all T11 dead stubs. Net −406 lines on `language-cortex.js` (3584 → 3178). `generate()` requires `opts.cortexCluster` — no fallback. T13.4/T13.5 partial (feedback + valence live; cerebellum transition predictor + motor channel filter deferred). |
| Persona Hebbian training | CompNeuro | Expert | **DONE — T13.1 2026-04-14** — `NeuronCluster.learnSentenceHebbian(embSequence, opts)` + `LanguageCortex.trainPersonaHebbian(cluster, text)` ship persona-corpus sequence Hebbian on the cortex cluster's internal synapse matrix. Inject word embedding → tick 3 LIF steps → snapshot spikes → Hebbian between consecutive snapshots (`ΔW_ij = η · curr_i · prev_j`, only on existing CSR connections) → Oja saturation decay per sentence. Logs before/after synapse stats (mean/rms/maxAbs/nnz). Console diagnostic via `cluster.diagnoseReadoutForEmbedding(emb, ticks)`. Called during boot after `innerVoice.loadPersona(text)`. Persistence via existing `SparseMatrix.serialize` path — trained weights survive restart. |
