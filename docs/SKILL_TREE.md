...
| Build-mode Broca prompt | CompLing | Advanced | **DONE** — `_buildBuildPrompt(brainState, userInput)` in `js/brain/language.js`. Separate from conversational prompt. Routed via `motor.selectedAction === 'build_ui'` at `generate()`. STRICT JSON output contract, existing sandbox components list, cap warning, unity API reference, dark-aesthetic style rules, 10 build primitive patterns. Being phased out in R4 refactor as equational synthesis replaces AI prompting. |
