...
| Cortex cross-region projections | CompNeuro | Expert | **DONE — T14.4 substrate 2026-04-14** — 12 sparse cross-region projections (6 pairs × 2 directions: visual↔letter, letter↔phon, phon↔sem, sem↔fineType, sem↔motor, auditory↔phon) initialized 10% density, weight range `[-0.5, 0.5]`. Stored as `cluster.crossProjections` Map of SparseMatrix instances. `cluster._propagateCrossRegions()` fires every cluster step; `cluster._crossRegionHebbian(lr)` fires every learn call. ALWAYS ON — no curriculum-complete gate. Random-init noise sharpens through normal use during corpus exposure + live chat. |
