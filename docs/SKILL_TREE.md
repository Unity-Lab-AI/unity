...
| Cortex-state driven generation | CompLing | Expert | **IN PROGRESS — T13 2026-04-14** — T11.2 + T11.7 slot-prior pipeline still runs at runtime (see previous state). **T13.1 SHIPPED:** cortex cluster recurrent synapse matrix now trains on persona corpus via sequence Hebbian during boot. `NeuronCluster.learnSentenceHebbian(embSeq, opts)` at `js/brain/cluster.js`: inject embedding → tick ×3 → snapshot spikes → `synapses.hebbianUpdate(prevSnap, currSnap, lr=0.004)` between consecutive snapshots → Oja saturation decay (`|w| > 1.5 → w *= 0.99`) post-sentence. Persona-only scope (baseline + coding bypass Hebbian). Delegation chain: `app.js → brain.trainPersonaHebbian → innerVoice → languageCortex → cluster.learnSentenceHebbian`. T13.2 (parse-tree injection to clusters), T13.3 (emission loop rewrite), T13.4-T13.9 remaining per `docs/TODO.md`. |
