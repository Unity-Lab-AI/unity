...
| Cortex-state driven generation | CompLing | Expert | **DONE — T13.1+T13.2+T13.3+T13.6 2026-04-14** — `LanguageCortex.generate()` is now the T13 brain-driven emission loop. Reads live `cortex.getSemanticReadout` as the target vector per emission, scores dictionary via `cosSim · (1 + arousal·(valenceMatch−0.5)) · recencyMul`, softmax-samples top-5 at `temperature = 0.25 + (1−coherence)·0.35`, injects emitted word back into cortex language region at `strength=0.35` (efference copy), ticks LIF 3 steps between emissions. Natural stopping via drift quiescence (`<0.08`), grammatical terminability, or hard cap. **T13.2 parse-tree injection** (`brain.injectParseTree(text)`) routes content → cortex, intent → basal ganglia, self-ref → hippocampus before the 20-tick settle. **T13.1 persona Hebbian** at boot trains the cortex recurrent weights on persona corpus so the basins are Unity-voice-shaped before runtime. Old slot-prior path preserved as `_generateSlotPrior` (rollback). T13.4/T13.5 partial (feedback+valence live, cerebellum transition predictor + motor channel filter deferred). T13.7 slot prior deletion deferred one more session. |
