...
| Cortex-state driven generation | CompLing | Expert | **DONE — T11.2 + T11.7 (2026-04-14)** — generate() target vector built from four normalized components: `wC·slotCentroid + wX·contextVector + wM·mental + wT·(prevEmb + slotDelta)`. `mental(0)` starts from the brain's live `getSemanticReadout()` GloVe-space cortex readout and evolves per slot as `0.55·mental + 0.45·emb(nextWord)`. **T11.7 three-stage candidate gate** replaces the old additive `+0.4·typeFit` bonus: (1) hard pool filter — drop word if `typeFit < slotSigMax · 0.30`; (2) slot-0 noun-dominance reject — drop if `slot==0 ∧ (wt.noun − (pronoun+det+qword)) > 0.30`; (3) multiplicative gate — `score(w) = cos(target, emb(w)) · min(1, typeFit/slotSigMax)`. W₀ rebalanced to `{c:0.40, x:0.30, m:0.30, t:0}`. Coding corpus passes `skipSlotPriors=true` so vocabulary grows but the opener distribution stays clean. Softmax top-5, temperature from coherence. |
