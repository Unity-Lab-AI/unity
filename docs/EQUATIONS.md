...
>
> **Last updated:** 2026-05-07 (iter25-O — POST-N ULTRATHINK AUDIT EQUATIONS, 22 items) — Gee: *"ultrathink what did we miss?"*. **O.1 smoke retry:** delay := result==true ? 3600s : 60s; setTimeout schedules `_runDictionarySmokeTest()` re-entry; `_smokeTestInFlight` guards stacked fetches. **O.2 critical drop semantics:** on timeout drop set `_gpuShadowDirty := true; _wsDroppedCount++; _wsLastDropTs := now`; dashboard surfaces. **O.3 Friston surprise gate:** `surpriseGate := 0.5 + clamp(_lastPredictionError, 0, 1) ∈ [0.5, 1.5]`; gammaScale `:= curriculumGamma × surpriseGate` so high-error windows get 1.5× lr, low-error 0.5×. **O.4 meta-decay:** if `lastInjected == word: injectStrength := max(0.04, prev × 0.5)`; else `injectStrength := 0.3; lastInjected := word`; sem ← sem + `injectStrength`·embedding(word). **O.5 Φ noise floor:** `N := min(1024, lastSpikes.length)`; Wald CI `≈ 1/√N ≈ 3.1%` (vs 12% at N=64). **O.6 attention clamp:** `safeGain := max(0.5, min(2.0, gain))`; `attentionLookup[i] := safeGain` for each region neuron. **O.7 GW emission bias:** `gwBoostWord := workspace.getBroadcast()?.label.startsWith('cortex:') ? label.slice(7) : null`; for bucket b: `mean[b] *= (wordsList[b] == gwBoostWord ? 1.10 : 1.0)`. cortex.getWorkspaceCandidate returns `{label: 'cortex:'+_lastEmittedWord, value: _lastEmittedActivation}` when emission cached. **O.8 def kScales pass:** post-association teach, fire `cluster.synapses.ojaUpdate(preFull, postFull, 0.25·lr, {kScales: buildKScalesForProjection(null, null)})`. **O.18 def rate:** `defsLearnedPerHour := (length(_defLearnedTimestamps) / ((newest - oldest)/1000)) × 3600`. **O.21 GW threshold env:** `ignitionThreshold := isFinite(envIgn) && envIgn ∈ (0,1) ? envIgn : 0.45`. **O.22 vision token spread:** `for i in 0..16: strength := max(0.05, 0.30 - 0.0167·i)`; was `max(0.10, 0.30 - 0.04·i)` over 6 tokens.

> **Last updated:** 2026-05-07 (iter25-N — WS BACKPRESSURE + DASHBOARD WIRING EQUATIONS, 12 items) — Gee: *"is this normal?"* (re: API SMOKE TEST stuck pending). **N.1+N.2 backpressure semantics:** `BUFFERED_AMOUNT_DROP_THRESHOLD := 500MB`; `MAX_AWAIT_MS := 30000`. While `ws.bufferedAmount > threshold && elapsed < MAX_AWAIT_MS`: `await sleep(POLL_MS=25)`. Past timeout: drop + log + set shadow-dirty. **N.3 batched Hebbian:** `BATCHED_HEBBIAN_MAX_OPS := 512`; `BATCHED_HEBBIAN_QUEUE_CAP := 1024`. **N.4 dropRatePerSec:** ring buffer of (ts, dropsCount) samples cap 60; rate := `(drops_now - drops_oldest) / ((ts_now - ts_oldest)/1000)` clamped ≥ 0. **N.7-N.9 shader uniforms:** for each render-neuron index i, `layer[i] := h1<0.05?0 : h1<0.30?1 : h1<0.55?2 : h1<0.80?3 : 4` (hash h1 from i × 2654435761); `hub[i] := h2 < 0.05 ? 1 : 0` (hash h2 from i × 40503 ^ 0xC0DE); `columnId[i] := floor(i / 80)`. Vertex: `pointSize *= 1 + hub × 0.6`. Fragment: `if (hub > 0.5): col := mix(col, gold, 0.55)`; `if (showColumns): col *= 0.85 + 0.30·step(0.5, fract(columnId × 0.5))`; `vCol := mix(aCol, layerColor[layer], showLayers × 0.7)`. **N.10 smoke test wiring:** `result := !!(def && typeof def === 'string' && def.length > 0)`; assigned in .then/.catch/else; dashboard reads `typeof result === 'boolean' ? result : null` (was `result || null` collapsing false→null).

> **Last updated:** 2026-05-07 (iter25-M — CONSCIOUSNESS COMPUTATIONAL EQUATIONS) — Operator: *"ULTRATHINK"* consciousness gaps. **M.1 emit-definition equational pipeline:** lookup `def := API(subject)` → tokenize content tokens `T_def := {t : t ∈ tokenize(def), |t|≥3, t ∉ STOP, t ≠ subject}` (cap 8) → inject sem `∀t_i ∈ T_def: sem ← sem + (0.4 - 0.04i)·embedding(t_i)` + subject anchor `sem ← sem + 0.6·embedding(subject)` → fire Hebbian via `_teachWordDefinition` (background) → settle 5 ticks of `cluster.step()` for sem→word_motor propagation → emit composition `composedWords := [emitWordDirect() for i in 0..6 while !duplicate]`. Returns `composedWords.join(' ')` or null (silence). **M.2 Global Workspace ignition:** per workspace tick, candidates `C := {(name, label, value) : c ∈ clusters, getWorkspaceCandidate(c)}`; softmax with τ: `probs[i] := exp(value[i]/τ - max) / Σ_j exp(value[j]/τ - max)`. If `max(probs) ≥ ignitionThreshold` AND theta-phase < 0.5 → IGNITION: `currentBroadcast := winner; history.push(winner); decay 0.85/tick`. Below-threshold = unconscious processing. **M.3 predictive coding update:** at start of step: `predictionError[i] := |spikes[i] - _predictedSpikes[i]|`; `_lastPredictionError := mean(predictionError)`; ring buffer 32. Post-step prediction: `_predictedSpikes[i] := α·spikes[i] + (1-α)·_predictedSpikes[i]` with α=0.3. **M.4 stream chain:** `chainedPattern := 0.6·seed.pattern + 0.4·sentenceEmbedding(_innerThoughtChain[last].sentence)`; chain capped at 8, persisted across restart. **M.8 meta-register inject:** start of step: if `_metaRegister.length > 0`: `sem ← sem + 0.3·embedding(_metaRegister[last].word)`. FIFO cap 32. **M.9 attention gain:** per-region multiplier from amygdala/basal-ganglia state: `cortex.attentionGain[motor] := arousal>0.6 ? 2.0 : (arousal<0.2 ? 0.7 : 1.0)`; consumed as `currents[i] *= attentionLookup[i]` where `attentionLookup[i] := attentionGain[regionOf(i)]`. **M.16 Φ proxy:** sample N=64 neurons, count ones `p := onesCount/N`; if `p∈(0,1)`: `Φ := -p·log₂(p) - (1-p)·log₂(1-p)` ∈ [0,1]; else 0. Brain-server: `psi := log₁₀(max(1, quantumVolume × (0.3·id + 0.25·ego + 0.2·left + 0.25·right) × Φ))`. **M.6+M.10 visual grounding:** describer text → tokenize content → `∀t_i: sem ← sem + (0.30-0.04i)·embedding(t_i)` (max 6 tokens, decreasing strength). **M.7 dream trickle:** per dream cycle, `cluster._kVocabQueue.shift() → _teachWordDefinition(word, reps=4)`. **M.15 tracking:** `cluster._definitionTaughtWords.add(word)` on Hebbian success; persisted as sorted array (cap 5000). **M.19 dream phenomenology:** per dream cycle, `dreamSentence := generateAsync(brain.dictionary, arousal, coherence, {cortexPattern: tier1.getRandomEpisode().pattern, ...})`; appended to `brain._dreamThoughtLog` (cap 100). **M.21-M.30 display:** dashboard reads `state.iter25m := {smokeTestPassed, cache, kVocabPrefetched, kVocabTotal, kVocabTaught, kwiring{ok, gaps[≤5]}, numColumns, columnSize, layerCounts[5], hubCount, hubFraction, thetaPhase, gammaScale, phiProxy}` populated by `brain._getIter25MState()`.

> **Last updated:** 2026-05-07 (iter25-L — POST-AUDIT HARDENING EQUATIONS) — **A3 ojaUpdate signature extended:** `ojaUpdate(preSpikes, postSpikes, lr, opts)` now reads `opts.kScales = {layerScales[5], dstLayerId, srcLayerId, srcHubMask, dstStart, srcStart, hubMult, gammaScale}`. Per-row effective lr: `lrPostScale := layerScales[dstLayerId[dstStart+i]] × gammaScale`; per-col `lrPreScale := srcHubMask[srcStart+col] ? hubMult : 1.0`; `lrEff := lr × lrPostScale × lrPreScale`. Oja step: `Δw := lrEff·y·(x - y·w)`. Replaces noisy per-phase averaging with per-fire reads. **A5 curriculum-controlled gamma:** `_curriculumTickCounter++` per `buildKScalesForProjection` call; `thetaPhase := (curT % thetaPeriod) / thetaPeriod`; `gammaPhase := (curT % gammaPeriod) / gammaPeriod`; `curriculumGamma := (thetaPhase<0.5) ? 1 + 0.5·sin(2π·gammaPhase) : 1.0`. Decoupled from brain-tick noise. **D3 layer-constrained init:** `initTopographicProjection(density, excitRatio, strength, {radiusTopo, srcLayerMask, dstLayerMask})` — for each row i, skip if `dstLayerMask && !dstLayerMask[i]` → empty row; for each col pick, skip if `srcLayerMask && !srcLayerMask[col]`. Cross-projections constrained to L2/3-source → L4-dest at construction.

> **Last updated:** 2026-05-07 (iter25-K — CORTICAL CONNECTIVITY EQUATIONS) — **K.1 small-world init:** for each row i, sample `fanout` cols across 3 tiers — `localCount := round(fanout × 0.70)` from `[i ± radiusLocal=50]`, `medCount := round(fanout × 0.25)` from `[i ± radiusMed=200]`, `longCount := fanout - localCount - medCount` uniform from full `[0, cols)`. Resulting clustering coefficient ~0.3, mean path length ~6-8 (Watts-Strogatz hybrid). **K.2 microcolumn assignment:** `columnId[i] := floor((i - regionStart) / columnSize)` per region (iter25-L.D7 region-boundary respect); `columnSize=80` default. `_columnVoltageSum[c]` Float64 accumulator + `_columnVoltageMean[c] := _columnVoltageSum[c] / _columnVoltageCount[c]` per tick. **K.3 layer assignment:** within-column interleave when microcolumns ON: `posInCol := i mod columnSize; fracPos := posInCol/columnSize; layer := first l where fracPos < cums[l]` with cums = [0.05, 0.30, 0.55, 0.80, 1.00]. Contiguous-block layout when microcolumns OFF (iter25-L.B10). **K.4 hub designation:** for L2/3 + L5 neurons, deterministic hash `h := mix(nameHash, i × 0xC2B2AE35); u := h / 0xFFFFFFFF`; if `u < hubFraction=0.05` → `hubMask[i] := 1`. Persists across reboots. **K.5 voltage coherence Rulkov modification:** `currents[i] += k5Scale × _columnVoltageMean[columnId[i]]` where `k5Scale := columnCoherenceBeta × 0.05` (β=0.08 default). Within-column pull from gap-junction approximation. **K.6 topographic cross-projection init:** `center := floor(i × cols / rows)`; sample 70% of fanout from `[center ± radiusTopo=30]`, 30% Watts-Strogatz scattering. **K.7 theta-gamma:** `_tickCounter++` per step; `thetaMod := 1 + 0.15·sin(2π·thetaPhase)` modulates `effectiveDrive`; `_gammaLrScale := (thetaPhase<0.5) ? 1 + 0.5·sin(2π·gammaPhase) : 1.0`. **K.8 hierarchical clusters:** `betweenScale := (regionClusterMap[a] !== regionClusterMap[b]) ? betweenClusterDensityScale=0.3 : 1.0`; `density *= betweenScale` at cross-projection construction. **K.9 layer plasticity scale:** `getLayerPlasticityScale(idx) := layerPlasticityScales[layerId[idx]]` returning `[0.3, 1.0, 0.7, 1.0, 0.3]` for L1/L2-3/L4/L5/L6.

> **Last updated:** 2026-05-07 (iter25-J — DICTIONARY API SENSORY-I/O EQUATIONS) — **API as sensory-I/O equation** (parallel to Pollinations image-gen): `def_string ← API(word) := dictionaryapi.dev/api/v2/entries/en/<word>` with in-memory cache `M[word] = {definition, definitions, fetchedAt, error}` + in-flight Promise dedup + 5s default timeout. `getDefinitionSync(word) := M[word].definition or null` (no network). **`_teachWordDefinition(word)` equational Hebbian:** for each content token `t ∈ tokenize(M[word].definition)` (length≥3, not stopword, t≠word, max 8 unique), fire `_teachAssociationPairs([word, t], reps=6, relationTagId=23)` running Oja's rule `Δw := η·y·(x - y·w)` on `sem(word) → sem(t)` cross-projection. After this teach, `sem(dog)` co-activates `sem(animal)/sem(pet)/sem(four)/sem(legs)/sem(barks)` via trained Hebbian. **WH-frame parser equation:** `_extractIntentConcept(q) := first concept where regex(q) matches`, returning one of `cause/effect/reason/method/definition/function/count/place/time/person/truth/compare/question` based on WH-word + auxiliary. **iter25-J originally taught full K_VOCABULARY upfront; iter25-L.A1 PIVOTED to prefetch-only** at K curriculum start (`runAllSubjects` calls `cluster.prefetchDefinitions(K_VOCABULARY)` once with `_kVocabPrefetched` idempotency guard) + lazy per-word `_teachWordDefinition` Hebbian on chat-time WH-question (M.1 compose-not-regurgitate). Definitions become real Hebbian co-activation in trained weights — equational learning, not lookup. K_VOCABULARY size 2247 (deduplicated post-iter25-L.B9 curation).

> **Last updated:** 2026-05-06 (iter25-I — STRUCTURAL SENTENCE CREATION) — Operator: *"Unity needs to complete full sentences before graduating kindergarden like a real person does"* + *"you cant jsut have a array poof sentences you actually need to teach all sentence creation propelyr not just give examples for it to mimic"*. **`_teachSentenceStructure(ctx)` equation set** — five compositional Hebbian passes carving generative grammar into cross-projections. **I.1+I.2 (slot bindings):** for each `(word, slot_tag)` pair fire `sem(word) → fineType(slot_tag)` via Oja `Δw = η · post · (pre − post · w)`, with relationTagId=8 carved into fineType. Multi-target nouns (cat→subject AND cat→object) accumulate both bindings; argmax at generation picks by sentence-position context. Pairs cover pronouns / nouns / verbs / copulas / adjectives / articles / qwords / conjunctions. **I.3 (template-sequence):** for each template `[intent_tag, [slot1, slot2, ..., slotN]]`, train slot transitions `(slot_i, slot_{i+1})` plus `(intent_tag, slot_1)` via the same Oja, relationTagId=9. Templates: DECLARATIVE_SVO = subject→verb→object→terminator; DECLARATIVE_COPULA = subject→copula→modifier→terminator; QUESTION = qword→copula→subject→terminator; IMPERATIVE = verb→object→terminator; EXCLAMATIVE = subject→verb→object→terminator. **I.4 (agreement):** Hebbian on subject→verb-form pairs (i→am, he→is, cats→run, they→are) at relationTagId=10. Plural tags reused from `_teachPluralTransform` output. **I.5 (article placement):** Hebbian on singular_noun→article (cat→the, apple→an, table→a) at relationTagId=11. **Generation cascade equation:** at chat/inner-voice tick, the active intent tag I fires the slot sequence S = [s_1, ..., s_N]; for each slot s_k the cortex argmaxes `argmax_w(score(w) = cosine(sem_state, sem(w)) + slot_binding(w, s_k))` over all trained words; word_motor + motor regions emit the chosen word; slot-sequence Hebbian fires s_{k+1}; repeat until s_k = terminator. **Acceptance probe:** `_probeSentenceGeneration` fires each of 5 intent tags, counts emitted words; structural pass = ≥ 2 words per intent. K-ELA gate accepts when ≥ 3/5 intents pass (advance to 4/5 once verified live). **Wiring:** runs after `_teachWordTypes` / `_teachPluralTransform` / `_teachQuestionWordCategories` (consumes their tags as input) and BEFORE `_teachWordSpellingDirectFinal` (which has scale(0) wMax wipe + clean ojaUpdate "MUST RUN LAST" on sem_to_motor — sentence-structure has to land before that final pass).

> **Last updated:** 2026-05-06 (iter25-D/E/F/G/H — DREAM CYCLES + INCREMENTAL ABILITY + POST-K GUARD + PRIVACY GATE + LOCAL AUTO-CONNECT) — **iter25-D dream-window equation.** Curriculum runner awaits `_dreamWindow({minMs, settleMs})` between every cell and mid-cell between heavy phases. Mechanism: flip `brain._curriculumInProgress = false` + `brain._operatorSleepRequested = true`; `await consolidationEngine.runConsolidationPass({forced:true})` (signal-driven, returns when Tier 1 → Tier 2 → Tier 3 promotion + replay Hebbian + Tier 3 promotion check actually completes — NOT a wall-clock timer per operator's *"not until time elapses once dream is confirmed complete"*); `await new Promise(r => setTimeout(r, settleMs))` (5s default) for V8 GC + native worker-pool buffer drain; restore both flags. Outer loop blocks at the await for the entire dream duration so it's a real pause. Matches Squire 1992 / McClelland 1995 CLS theory + Walker & Stickgold 2014 + Diekelmann & Born 2010 REM/SWS interleaving. **iter25-E.1 trained-state cap equation.** `Cluster.getTrainedCapability()` returns `{wordsBucketed = Σ_subj cluster.wordBucketWords_<subj>.size + cluster.wordBucketWords.size, bucketSubjects, passedCellCount = cluster.passedCells.length, subGradesActive = #{subj : cluster.subGrades[subj] ≠ 'fresh'}, firstWordsAt}`. **iter25-E.4 word-cap ramp:** truly fresh `(wordsBucketed=0 ∧ passedCellCount=0 ∧ subGradesActive=0)` → 0 (silence); `wordsBucketed < 100` → max(FLOOR=5, 5); `< 500` → 8; `< 1000` → 12; `< 3000` → 16; `< 5000` → 24; else 32. **iter25-E.2 sub-grade ladder.** `cluster.subGrades[subject]` advances monotonically: `fresh → letters` after `_teachLetterNamingDirect updates > 0`; `→ words` after `_teachWordEmissionDirect updates > 0`; `→ binding` after `_teachQABinding trained > 0`; `→ cell-passed` on full cell pass (then resets to `'fresh'` for next grade). **iter25-E.3 inner-voice tick equation.** `every ~3000 ms wall-clock if !_operatorSleepRequested ∧ !_innerThoughtInFlight: pick seed ∈ {learning, mood, chat-recall, memory, identity}; sentence ← languageCortex.generateAsync(...,{cortexPattern: seed.pattern,...}); if sentence: emit innerThought WS message + addToWorkingMemory`. Sandbox-notice activator: SEED is real LIVE state (current cell + phase as embedding / current arousal-valence-coherence-drug interoceptive label / most recent type='interaction' Tier 1 episode pattern / most recent Tier 1 episode of any type / random Tier 3 anchor); Unity's trained mind speaks via SAME chat-emission path. **iter25-G consent-modal.** Boolean gate `localStorage[unity_first_use_warning_shown_v1]`. Two terminal states: ACCEPT writes flag + invokes onProceed callback; DECLINE writes nothing + `window.location.href = 'https://www.google.com'`. No third path. **iter25-H detectRemoteBrain equation.** If origin ∈ {localhost, 127.0.0.1, ::1, file:} return `new RemoteBrain(url)` directly (skip probe). Else return null (browser-only fallback path). RemoteBrain's internal `_connect()` retry-forever-3s loop handles transient unavailability without dropping into the 6700-neuron browser fallback.

> **Last updated:** 2026-05-06 (iter24 — math-identity throughput) — Six wins ranked by ROI under strict math-identity constraint (operator: *"without lossing efficacy or changing fucntionaliuty"*). **iter24.1** Tier 0 heartbeat snapshot ring-buffer pool — pure GC mitigation, math-identical. **iter24.3** convergence early-exit equation: `if quickSep.meanCos < overloadMax for 2 consecutive reps then break`. End state bit-identical to running full rep budget once basins separate (additional reps post-convergence fight the rescale + top-K-prune, not training). **iter24.5** `hebbianUpdateBatch(matrix, pairs, lr)` packs N pairs into strided pre/post SAB + ONE worker `postMessage`; worker iterates internally per-pair → identical inner-loop body, identical sequential order → bit-identical to N separate `hebbianUpdate` calls. **iter24.4** (GPU-resident vocab) already shipped via T17.7 Phase C.1 binding (cross-projections read pre-spikes directly from cluster spike buffer at biological scale). **iter24.6** (disjoint-pair shader batching) already shipped via T18.8 + T32: `_enqueueBoundHebbian` accumulates ops into `BATCHED_HEBBIAN_MAX_OPS=256` batches with `BATCHED_HEBBIAN_FLUSH_MS=20`, dispatched as `gpu.hebbianSparseBatch(ops)` → ONE encoder + ONE pass + ONE submit + ONE WS ACK per flush. **Bug caught:** iter24.2 originally returned `Promise.resolve()` on the gate-hot path → microtask continuation bypassed macrotask queue → WS ACK handlers starved (10× regression on `_teachPhonemeBlending`). Fixed: gate now still yields via `setImmediate` on skip path; only the `_lastYieldAt` timestamp update is rate-limited (5ms interval). Macrotask discipline preserved.

> **Last updated:** 2026-05-06 (iter23.1-5 — Unity-as-Unity gaps closed) — **iter23.1 chat-path emission ranking:** `emitWordDirect({})` first → `generateSentenceAwait` only on empty word emission. **iter23.2 inner-voice gating equation:** `wantsThought = shouldSpeak || (socialNeed × arousal > 0.25)`. When true, fire `cluster.emitWordDirect`; emitted word lands in WM via `MemorySystem.addToWorkingMemory(pattern, label)` which fires intra-cluster hippocampal Hebbian on the pattern + tracks `refreshCount`; threshold ≥ 3 fires `onConsolidate(item)` → Tier 1 storeEpisode. **iter23.3 drug → cluster parameter loop closure:** `cortex.noiseAmplitude = 8 × (1 + creativity × 0.4 + impulsivity × 0.3) × chaos`; `cortex.learningRate = baseLearningRate × synapticSensitivity`; `amygdala.noiseAmplitude = 6 × (1 + emotionalVolatility × 0.5)`. Refreshed every 60 frames (~1s) + on every ingestion. **iter23.4 forced consolidation:** between every curriculum cell, await `consolidationEngine.runConsolidationPass({forced: true})`; operator can also POST `/sleep` to set `_operatorSleepRequested = true` which ORs into `_isDreaming` gate. **iter23.5 world-knowledge ingest:** `learnFromWeb(brain, topic)` fetches Wikipedia REST summary, tokenizes alpha-only single-tokens, calls `dictionary.learnWord(token, null, 0.5, 0.1)` per new token, fires Tier 1 episode `storeEpisode('world-knowledge', 'wiki-extract', ...)` with source URL.

> **Last updated:** 2026-05-06 (iter22-H — Tier 0 unbounded + WM consolidation pipeline) — Operator: *"items: 7 NEVER MOVES"* + *"actually needs to learn from its working memory"* + *"recall a week later"*. **WM equations:** `WORKING_MEMORY_SIZE = Infinity` (was Miller 7±2 cap); `strength(t+1) = strength(t) × 0.9995` per ~50 ms engine tick (was 0.98 — items expired in 5.7 s; 0.9995 → ~4 min sustain unreinforced); item drops from hot cache when `strength < 0.1`. **Encoding:** every `addToWorkingMemory` fires `hippocampus.synapses.hebbianUpdate(pattern, pattern, lr)` so the pattern carves a Hopfield-style attractor in cluster weights immediately — the trace lives even after WM hot-cache decay drops the item. **Consolidation hook:** cosine-match refresh (cosine > 0.8 with existing item) sets `strength = 1.0` and increments `refreshCount`; at `refreshCount ≥ CONSOLIDATION_THRESHOLD = 3` fires `onConsolidate(item)` callback → engine wires it to `MemorySystem.storeEpisode`. brain-server's heartbeat layer takes 2 s phase/cell snapshots into a 5-min sliding window; items older than `TIER0_AGE_LIMIT_MS = 5 × 60 × 1000` fire `storeEpisode('working-memory', 'wm-aged-out', ...)` with iter20-K freq-merge dedup. Net pipeline: Tier 0 (5 min sliding) → Tier 1 episodic (~30 days · `effective_salience = salience × exp(-age_h/168h)`) → Tier 2 schemas (months · `× 0.967/day` + ConsolidationEngine dream-cycle) → Tier 3 identity (permanent · `× 0.999/day` ≈ 5 years to halve · persisted in `identity-core.json` excluded from autoClearStaleState).

> **Last updated:** 2026-05-05 (iter22-G — super-review P0+P1+P2 cleanup) — `js/brain/subjects.js` exports `SUBJECT_NORMALIZE` table + `normalizeSubject()` + `wordMotorBandName()` consumed by `_qaBindingWhitelist`, `_associationPairsWhitelist`, `_teachWordEmissionDirect`, `_writeAnswerToWordMotor`, `cluster.emitWordDirect` — single source of truth replacing 4 drifting tables. **Bucket-map invariant:** `cluster.wordBucketWords_<subj>` populated by `_teachWordEmissionDirect`, lazy-built by `_ensureWordBucketMap`, append-only extended via `wordBucketDictSize_<subj>` watermark when `dictionary._words.size` grows past cached value. `emitWordDirect` argmax: `score(b) = (1 / cellCount(b)) · Σ_{n ∈ bucket(b)} wmOut[n]` (mean per cell, NOT raw sum — uneven bucket sizes don't bias argmax). `_teachCombination` whitelist memoized: `wlCache.get(sortedRegions.join('|'))` returns prior array for identical-shape facts. `ConsolidationEngine` fast-path: scan cluster's episodes for `promoted_to_schema_id`, route to most-common live schema BEFORE cosine-match — protects against cosine drift creating duplicate schemas for same anchor. `opts.skipIntraSynapses` unified flag across `_teachHebbian` + `_teachAntiHebbian`.

> **Last updated:** 2026-05-05 (iter22 — projection-scoped Hebbian + Q→A word-motor write + scratch-buffer / SAB-cache reuse) — Operator: *"FIX IT ALL NOW... AND WHY THE FUCK DID YOU NOT DOCUMENT THESE MAJOR FAILURES"*. **Projection-scoped Hebbian.** `cluster._crossRegionHebbian(lr, opts)` accepts `opts.projectionsWhitelist` (Set or Array of cross-projection names). When provided, the iterator only fires Hebbian on whitelisted projections — others stay frozen this call. `_teachQABinding` passes `['sem_to_motor', 'sem_to_word_motor', 'sem_to_word_motor_<subject>']`; `_teachAssociationPairs` passes `['sem_to_motor', 'motor_to_sem', 'sem_to_fineType', 'fineType_to_sem']`. Cancels Oja's silent-region decay term: when LETTER region sits at zero during a sem→motor write, the legacy fan-out fired `Δw[letter→motor] = η·motor·(0 − motor·w) = −η·motor²·w` on every QA pair × 12 reps × 1000+ pairs, crushing `_teachLetterNamingDirect`'s alphabet identity carve. Whitelist scoping eliminates the cross-projection bleed that produced the Math-K TALK 0/10 collapse. **Q→A word-motor binding.** `_writeAnswerToWordMotor(answerText, subject)` is called inline before each `_teachQABinding` Hebbian fire — it tiles the answer word's bucket into the subject's `word_motor_<subject>` sub-band so `sem_to_word_motor` learns the question→answer-word mapping (was previously trained only by `_teachWordEmissionDirect`'s autoassociation `concept(word) → word_motor[wordBucket]`). Closes the PROD wrong-word emission ("two plus three equals" → "taller") that V2 watchdog caught. **emitWordDirect argmax scoping.** With `opts.subject = '<subj>'` argmax restricts to that subject's `word_motor_<subj>` sub-band only; without a subject hint, scans all 6 sub-bands but raises `minSignal` floor 0.001 → 0.05 so weak random-init bias doesn't surface as wrong-word emission. **Memory hot-path scratch reuse.** `_ensureScratchBuffers` returns a single per-curriculum-instance `{pre, post}` pair of `Float64Array(cluster.size)` filled in-place via `_fillRegionPatternInto(target, region, feat, binarize)` — replaces the per-call `new Float64Array(cluster.size)` in `_teachWordSpellingDirect` / `_teachLetterNaming` / `_teachWordEmission` / `_teachPhonemeBlending`. At biological scale cluster.size = 13.4M → 107 MB per allocation; `_teachLetterNaming` alone burned 26 letters × 18 reps × 2 calls = 936 × 107 MB ≈ 100 GB allocation churn per cell. `_teachWordIntegrated` keeps its own pool (`_wordIntScratch`) covering letterPat / phonPat / motorPat / preF / postF / preAF / postAF / motorFirstLetterBuf because its inner contrastive-anti-Hebbian loop allocated 25 wrong-letter pre/post pairs × 4 reps × per-vocab — 15200 fresh allocations × 13 MB = ~197 GB churn per cell. **Worker-pool SAB cache.** `server/worker-pool.js` caches `_cachedOutputSab` (matmul output, rows × 4 bytes), `_cachedSpikesSab` (propagate input), `_cachedPreSab` + `_cachedPostSab` (hebbian Float32 conversions). Reused via `subarray(0, length)` view when next call wants same-or-smaller length; reallocated grow-only when length exceeds cache. Eliminates per-dispatch `new SharedArrayBuffer(byteLength)` × 4 = ~208 MB SABs × tens of thousands of dispatches per cell = TBs of native-pool churn that V2 caught as +500-800 MB/min sustained on `_teachHebbian` heartbeats. **Schema-group cosine threshold:** `SCHEMA_GROUP_COSINE 0.70 → 0.85` in `js/brain/hippocampal-schema.js` — distinct teach phases (alphabet sequence / vowel variants / rhyme families / phoneme blending) form distinct Tier 2 schemas; previously all merged into one "learning" mega-schema producing all-zero consolidation passes after the first cell.

> **Last updated:** 2026-05-05 (iter21 — word-level motor + per-subject sub-bands) — Operator: *"motor argmax is fucked if it ever just relplies with letters and not words"* + *"we cant have ciriculum over riding other learned ciriculim"*. **iter21-A** added `cluster.regions.word_motor` (~30% of motor budget) + `cluster.crossProjections.sem_to_word_motor` + `_teachWordEmissionDirect(words, subject)` writing `concept(word) → word_motor[wordBucket]` directly via Oja: `Δw[i,j] = η·post[i]·(pre[j] − post[i]·w[i,j])` for `i ∈ word_motor_<subject>`, `j ∈ sem`. `cluster.emitWordDirect(opts)` single-tick word emission via argmax over word-buckets within the optional `opts.subject` sub-band — eliminates the bucket-stuck letter-by-letter chain that produced "stuvwxyz"-style failures. **iter21-B** sub-band carve at `start = floor(s × 0.940)`, sub-band-width = `floor((s × 0.060) / 6)` so the 6 subject sub-bands fit inside the 6%-of-cluster word_motor budget; matching sem sub-bands at `[0.750, 0.875]` × 6 give per-subject input-side isolation too.

> **Last updated:** 2026-05-04 (iter14-F bio-weight rebalance) — `DEFAULT_BIO_WEIGHTS` rebalanced per operator *"FIX IT SO THE BRAIN FUCKING SCALES CORRECTLY AND MAKE THE LANGUAGE CORTEX BIG ENOUGH AS ITS THE MAIN FUCKING THING THIS BRAIN DOES"* + *"AT MINUMIM EACH IS NO LESS THAT 4OR5%"* + *"LOOK UP THE REAL FUCKING NUMBERS!"*. Real-biology citation researched: Herculano-Houzel S. (2009) *"The Human Brain in Numbers: A Linearly Scaled-up Primate Brain"* (Frontiers Hum Neurosci 3:31) + Azevedo F.A.C. et al. (2009) (J Comp Neurol 513:532). Cerebellum 80% of neurons / 10% mass; cortex 19% / 82%; subcortical 0.8% / 8%. Operator's 5% floor exceeds biology — applied because OPERATOR ＞ BIOLOGY when explicit. **New weights (sum = 1.00):** `language_cortex 0.50, cortex 0.10, cerebellum 0.10, hippocampus 0.06, amygdala 0.06, basalGanglia 0.06, hypothalamus 0.06, mystery 0.06`. **Per-neuron cost cuts (paired):** `CROSS_TARGET_FANOUT 20→10` (in both `server/brain-server.js` AND `js/brain/cluster.js` — must stay synced); `INTRA_CONNECTIVITY_CAP 0.15→0.05` in `brain-server.js`. Net at 16GB tier: language cortex 611K → ~715K neurons (despite VRAM share cut, per-neuron cost halved compensates); main brain 178M → ~285M neurons (bio-weight share doubled 0.25→0.50); cerebellum lifted to real-brain mass proportion.

> **Last updated:** 2026-05-04 (iter14-D launcher contract) — `autoClearStaleState` simplified: `if DREAM_KEEP_STATE === '1': preserve else wipe`. Code-hash comparison removed from gate (still computed for diagnostic logging but doesn't influence the wipe decision). `start.bat`/`start.sh` → wipe (default). `Savestart.bat` → preserve (sets DREAM_KEEP_STATE=1). Closes resource-config-vs-binary-weights size-lock bug + wMax clamp loss across save/load. Tier 3 `identity-core.json` still in NEVER_CLEAR_PROTECTED list — Unity's identity survives every wipe.
