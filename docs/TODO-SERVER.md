...
- [ ] **Broca's as optional peripheral** — `brain.processAndRespond()` checks if `_brocasArea` exists. If yes, add human language to the output. If no, the brain still communicates — through the sandbox (building UI), through behavioral state (mood/energy/expression rendered by the client), through images (generated from cortex patterns), through gaze and attention (visual cortex), through action (BG motor output). Language is one channel. The brain has all of them. The model adds English. The brain already speaks in everything else.
