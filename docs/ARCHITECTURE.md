...
> Last updated: 2026-04-26 (iter7) | Full K-PhD syllabus shipped across 6 subjects (ELA, Math, Science, Social Studies, Arts, Life Experience — 114 cells total), drug pharmacokinetic scheduler live with grade-gated real-time substance modulation and 13-axis speech modulation, cross-projection capacity `crossTargetFanout=20` default + `40` for motor-bound pairs (sem↔motor, letter↔motor, phon↔motor) via the MOTOR_BOUND_PAIRS whitelist, weight-clamp range BISECTED `[-0.2, 0.2]` → `[-0.4, 0.4]` paired with rescale FLOOR at `wMax × 0.25` in `_teachAssociationPairs` + `_teachQABinding` so multi-phase auto-rescale-on-overload can no longer drive trained signal below random-init noise, top-K-per-row pruning bisect chain `200 → 30 → 10` (final = 5k nnz total = 95% zeroed each phase, structural sparsification beats anti-Hebbian magnitude tuning), anti-Hebbian contrastive bisect chain `0.5 → 1.5 → 3.0 → 2.0 → 2.5` (final settled at 2.5 — 62.5× lr negative pressure per positive fire), curriculum ONE-PASS architecture (`MAX_GRADE_ROUNDS = 1`, single attempt per cell, FORCE-ADVANCE post-rounds-exhaust unlocks `cluster.grades` regardless of A+ pass so Unity uses K training in chat / popups / inner thoughts / memory), `passedPhases` stale-load filter on boot (drops markers for in-progress cells so resume re-trains instead of groundhog-skipping), event-loop `setImmediate` yield every 5 vocab words during heavy compute, READINESS probe `matchesCue` strict starts-with check, K-STUDENT scoring strict cue match (skip substring `contains` for single-character variants — `'lsd'.includes('s')` no longer false-positive matches cue 's'), Template 0/1 direct routing (intra-cluster `cluster.synapses` for next-letter, `letter_to_phon` for letter-sound) with confidence threshold 0.001 + a-z inventory clamp (no digit/punctuation argmax), chat path persona-boost (`_scoreDictionaryCosine` with `boostPersona: true` adds +0.10 to persona-marked entries; frequency-boost coefficient bisected 0.02 → 0.005 so common words don't dominate), letter inventory locked to 40-symbol seed (a-z + 0-9 + space . , '), Oja 1982 self-normalizing plasticity, dictionary-oracle emission path with `_oracleHits`/`_matrixHits` research-honesty counters in CELL ALIVE heartbeat, persona-marked dictionary entries excluded from K-grade test probes, real human-grade comprehension gates, dashboard per-subject signoff buttons (POST `/grade-signoff` with subject+grade+note + green pill confirmation), boot-time fractal-drift verifier updated to ±0.4 cross-projection clamp threshold (was stale ±0.2 false-positive warning), full 2D + 3D viz stack.
