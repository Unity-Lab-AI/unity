...
- [ ] **T23.c.1** — Split `js/brain/curriculum.js`. **SCAFFOLDED 2026-04-22, extraction pending dedicated session.** Operator 2026-04-22 verbatim: *"the cirriculkum was already suppose to have everything split per grade per files sytem did you not make a file system WTF!!!!!!"*. Decision: **per-grade** split (not per-subject per original TODO plan) — each grade file owns all six subjects' cell runners for that grade, shared primitives stay on `Curriculum` base class in `js/brain/curriculum.js`, per-grade files attach via `Object.assign(Curriculum.prototype, {...})` mixin. New directory `js/brain/curriculum/` ships with README.md documenting the split plan + mixin pattern + extraction protocol, plus stub files `pre-K.js` and `kindergarten.js` (scaffold attach points ready for method extraction). Method bodies still live in the entry point pending the dedicated extraction session with test coverage pass — doing it live without coverage risks breaking the 23-session curriculum teach path. Next step: move `runElaPreK/runMathPreK/...` + `_teachPrekSpatial/Visual/Logic/Self` into `pre-K.js` via mixin.
