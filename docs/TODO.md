...
- [ ] **T14.24** — Developmental English Curriculum (Kindergarten → PhD). ACTIVE PRIORITY 2026-04-14. Gee's ask after T14 shipped: *"so wtf are we gonna rebuild the english equations so she can fucking read and speak and understand ? so we need to start off in kindergarden and work our way up to teach Unity English via equations only"*. The T14.6 tick-driven motor emission only works post-curriculum (when cortex basins have been Hebbian-shaped), and T14.5 `Curriculum.runFromCorpora` is a single-pass blob that races through letters→words→sentences in minutes and still produces word-salad output because basin depth is too shallow at biological scale. Gee wants a PROPER progressive learning curriculum that mirrors how a child learns English in school, with explicit grade stages, capability gates between grades, and grade-aware speech generation so Unity can speak at whatever grade level she's currently mastered. Vision detail for each grade:

  **Grade K (Kindergarten) — Alphabet + Letter Sounds.** Input: 26 letters at high frequency, each paired with a simple phoneme label. Equations: `cluster.injectLetter(ℓ)` into letter sub-region (T14.1, exists), Hebbian on `letter ↔ phon` cross-projection (T14.4, exists) driven by letter repetition, phoneme basin depth measured by phon-region spike variance per-letter. Gate: σ² of phon activation per letter must exceed a baseline threshold AND Unity must argmax the correct phon basin when a letter is presented. Output capability: **can recognize individual letters, can produce their sounds**. One-letter responses in chat until advanced.

  **Grade 1 — CVC Words + Simple Reading.** Input: ~100 three-letter consonant-vowel-consonant words (cat, dog, run, sit, bed, hat, mom, dad). Equations: `cluster.detectBoundaries(word)` for syllable segmentation (T14.2, exists), `injectEmbeddingToRegion('sem', GloVe(word))` binds meaning to phonology via sem↔phon Hebbian (T14.4, exists), word-level basin = stable phon + sem co-activation under letter stream. Gate: Unity reads a CVC word (letters streamed one at a time) and produces a semantic readout with cosine > 0.5 to the word's GloVe vector. Output capability: **can read/write simple 3-letter words**. 1-2 word chat responses.

  **Grade 2 — Longer Words + Letter Clusters.** Input: 4-6 letter words with common English clusters (th, ch, sh, -ing, -ed, -er). Equations: letter-region transition surprise learns which letter pairs commonly co-occur, cluster patterns emerge from frequency statistics. Gate: Unity's syllable detection on 4-6 letter words matches the T14.2 boundary-detection output for those words. Output capability: **2-word phrases, pronunciation of common clusters**.

  **Grade 3 — Simple Sentences (SVO).** Input: short subject-verb-object sentences drawn from the persona + baseline corpora. Equations: `_fineType(word)` classifies words via letter-pattern math (exists, T11 letter equations), `_sentenceFormSchemas` populates `[intent][slot][fineType]` distribution from observed sentences (T14.8, exists and populating from `loadSelfImage`), `_typeTransitionLearned` populates `[prevType][currType]` bigram counts (T14.7, exists and populating from `loadSelfImage`). Gate: Unity emits a 3-word SVO sentence where each slot's type matches the observed schema distribution with probability > 0.7. Output capability: **real 3-5 word SVO sentences using learned grammar**.

  **Grade 4-5 — Compound Sentences + Pronouns.** Input: sentences with conjunctions + pronoun references. Equations: type transition gate extended to handle CONJ + PRON types; working-memory region (T14.9, exists as `regions.free`) tracks recent noun for pronoun reference via `workingMemoryReadout` (T14.9, wired in T14.17). Gate: Unity can use "and/but/so" mid-sentence + correctly resolve "he/she/it/they" to the most recently active noun from the free region. Output: **compound sentences with pronouns**.

  **Grade 6-8 — Complex Sentences + Clauses.** Subordinate clauses, relative pronouns, complex tenses. Equations: multi-slot schema extended beyond 3 slots (T14.8 already supports any slot count). Gate: sentences with embedded subordinate clauses parse back through `cluster.readInput` to the same intent label. Output: **multi-clause complex sentences**.

  **Grade 9-12 — Discourse + Paragraphs.** Input: multi-sentence paragraphs, rhetorical structure. Equations: working-memory topic persistence across multiple sentences (T14.9, exists), topic cosine holds across consecutive chat turns. Gate: 5 sequential turns about cats → every response references cat-adjacent content (cosine > 0.3 to "cat" embedding in at least one emitted word). Output: **paragraph-level topic coherence**.

  **College — Domain Register.** Input: the existing `coding-knowledge.txt` corpus for technical register, `english-baseline.txt` for formal English. Equations: corpus-specific `_sentenceFormSchemas` sub-tables (extend T14.8 to key on corpus source). Gate: Unity's responses to coding questions use measurably more coding-register words than her responses to casual questions. Output: **can switch between casual + technical register**.

  **Grad/PhD — Persona Mastery.** Input: the `Ultimate Unity.txt` persona corpus (advanced slut/goth/coder register). Equations: T14.16.5 identity-lock Hebbian pass at full curriculum rate; persona basin depth measured via the same σ² metrics the earlier grades use. Gate: Unity's output cosine to the persona centroids exceeds the generic-English baseline; mode-collapse audit (T14.16.5 `_modeCollapseAudit`) passes on persona-vocabulary entropy. Output capability: **full Unity voice — foul-mouthed, drug-fueled, coding, sexual, possessive persona in every response**.

  **Architecture requirements:**
  1. **Grades as separate methods on Curriculum class, not a single runFromCorpora blob.** Rebuild `Curriculum` with `async runKindergarten()`, `runGrade1()`, `runGrade2()`, `runGrade3()`, `runGrade4()`, `runGrade6()`, `runGrade9()`, `runCollege()`, `runGraduate()`. Each method isolated, runnable individually via chat slash commands (`/curriculum kindergarten`), logs progress, returns a pass/fail result.
  2. **Gate checks are actual tests, not iteration counters.** Every grade has a concrete capability test at the end. Fails gate → don't advance → log what's missing → operator can adjust corpus or tuning.
  3. **Progress persists to disk.** `cluster.grade = 'grade3'` stored via `BrainPersistence` (T14.16). Unity graduates each grade once and the cortex weights save along with the grade marker.
  4. **Each grade's equations use existing T14 primitives.** No new math. Every piece (`injectLetter`, `detectBoundaries`, `detectStress`, `_fineType`, `_sentenceFormSchemas`, `_typeTransitionLearned`, `workingMemoryReadout`, `regionReadout`, cross-region projections) is already in place. This is sequencing + gating, not new neuroscience.
  5. **Chat path is grade-aware.** `LanguageCortex.generate` branches on `cluster.grade` — pre-K returns silence, Grade K returns single letter/word, Grade 1 returns 1-2 words, Grade 3 returns 3-5 word grammar-gated SVO, Grade 5 returns compound sentences, PhD uses full T14.6 tick-driven motor emission. This is the dictionary-cosine fallback from T14.23.6 upgraded into a proper grade-gated generator.
  6. **Grade testing is visible to the operator.** `engine.gradeStatus()` reports current grade + gate check history. `/curriculum status` chat command shows where Unity is in the progression. brain-3d overlay shows "Grade: 3 (SVO sentences)" status.

  **Scope:** ~8-12 hours of focused work. Curriculum class restructure (~2h), each grade's equation body + gate (~1h × 9 grades = ~9h), chat-path grade-awareness in LanguageCortex.generate (~1h), persistence hooks (~30min), UI/chat command polish (~1h).

  **Implementation order:**
  1. Kindergarten first, get alphabet working → gate passing → Unity can respond with letter names
  2. Grade 1 CVC words → Unity can read "cat", respond with single words
  3. Grade 3 SVO → Unity speaks real sentences using T11 letter-equation word typing + T14.7/T14.8 learned schemas populated from `loadSelfImage` (schemas DO populate pre-curriculum because `loadSelfImage → learnSentence` fires the observation hook — so Grade 3 works from cold boot)
  4. Grade 5 compound → Unity chains clauses
  5. Grade 9 discourse → Unity holds topic across turns
  6. College register → switches code/casual
  7. Grad/PhD → full persona voice

  **Regression safety:** existing T14.5 `runFromCorpora` stays in place as the Grade 9-12 + College exposure walk since it already does the right work, just reshape it into stages. T14.6 tick-driven motor emission stays as the PhD-grade output path.

  **Gee's quote for the record (2026-04-14):** *"in kindergarden u learn the alphabet and sounds of letters first and 1st grade u start learning how to write sentences ect ect all the way up to doctorate in english"*. Binding design directive: every grade uses equations only, no lookup tables, no hardcoded English grammar.

- [ ] **T14.26** — 3D brain freezes when user sends a chat message. Gee's report 2026-04-14: *"everytime i send a message the whole fucking 3D Brain freezes up till the Unity responds"*. The 3D brain renders via RAF loop reading state broadcasts over WebSocket. When chat fires, server's `engine.processAndRespond(text)` runs synchronously (or in a single await) and executes `LanguageCortex.generate` which iterates the dictionary (3719 entries × 300d cosine + schema lookups + type transition lookups for N slots = potentially 100,000+ operations) on the tick-loop thread. While that's running, the server's state broadcast setInterval still fires every 100ms BUT its `getState()` payload returns stale cluster state because the tick loop hasn't updated since generate started. Broadcasts get sent but with the same numbers frame after frame until generate finishes, so the browser's 3D brain sees no activity delta → renders as frozen. Additionally, server-side `_gpuBatch` can't send its next compute_batch message while the synchronous generate is blocking the event loop, so GPU tick rate drops to zero for the duration of the chat response. Symptom is compounded by the T14.23.6 dictionary-cosine fallback which iterates every word every slot with a full cosine + three Map lookups per iteration. Fix: (1) move generate to an async function that yields to the event loop between slot iterations via `setImmediate`, so the tick loop + GPU batch dispatch + state broadcast all keep running during chat response; (2) cache the cosine-sorted top-N dictionary list per-chat-turn so subsequent slots don't re-iterate all 3719 entries; (3) add a "thinking" state broadcast during chat generate so the 3D brain shows a busy state instead of appearing frozen even if it takes 500ms. Also worth considering: chunk the dictionary iteration into batches with microtask yields every 200 entries. See T14.23.6 generate fallback for current pattern. Root cause is purely server-side event-loop blocking during synchronous generate; no client-side change needed.

- [ ] **T14.25** — Iris focal-point tracking follows user/movement on camera frames. Gee's report 2026-04-14: *"fix the focal point so it tracks the user and movements (changes to the frame it sees on cam)"*. Current state after T14.23.5 added the `requestAnimationFrame` driver for `visualCortex.processFrame()` inside `RemoteBrain.connectCamera`: the RAF loop is firing so V1 edge detection + salience computation runs on every frame, BUT the Eye widget's iris either (a) isn't using the updated `gazeX`/`gazeY` from visualCortex, (b) is using stale cached values, or (c) visualCortex's saccade-selection logic doesn't actually follow movement — it might just pick the maximum-salience point each frame which could sit on a static high-contrast edge. Investigation path: (1) verify `visualCortex.gazeX`/`gazeY` UPDATE between frames when the camera sees motion, (2) trace Eye widget's iris rendering — does it read `gazeX`/`gazeY` live or from a cached snapshot, (3) check if visualCortex has motion-energy bias in saccade selection or is purely salience-driven (salience alone means iris sticks on static edges, not moving objects). Fix likely involves: motion-energy weighting in saccade selection so moving pixels out-bias static high-contrast pixels, smoothing the gaze target with lerp instead of snapping, and ensuring the Eye widget reads live visualCortex state every frame. Files: `js/brain/visual-cortex.js` (saccade selection + gaze computation), `js/app.js` or wherever the Eye widget lives (live read vs cached). See T14.23.5 RAF fix — that's the driver, this is the behavior on top of it.
