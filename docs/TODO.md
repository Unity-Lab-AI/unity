...
3. **T17.7** — Single-cortex integration — **Phases A + B + C + D + E.a + E.b SHIPPED Sessions 114.19y/z/aa/ac** (foundation substrate + dual-cortex bridge with Ψ-modulated hemispheric gating + biological-scale sensory injection + per-tick spike mirror + divergence → cerebellum correction + shared-infrastructure curriculum migration via `_writeTiledPattern` forwarder + cross-projection rebind + cluster-bound Hebbian/propagate + GPU-native sparse spike path + Phase D motor-slice letter-bucket reduction + generateSentenceAwait reads argmax from main cortex + Phase E.a intent-injection forward via writeCurrentSlice path + **Phase E.b workingMemoryReadoutAwait via bucketed reduction over main-cortex free slice**). **Phase E remaining pieces + F still open:**
   - ~~**Phase C follow-up**~~ — **SHIPPED Session 114.19ac** — `state.cortexDivergenceByRegion` broadcasts per-region {standRate, mainRate, divergence} map so Gee can inspect WHICH region is slipping during K walk, not just a cluster-wide scalar.
   - **Phase E.c** — delete `_mirrorCortexRegions` per-tick bridge (safe after all generation paths read main cortex authoritatively; generateSentenceAwait does after E.a+E.b; other paths via cortexCluster still depend on mirror)
   - **Phase E.d** — delete standalone cortexCluster construction + persistence VERSION bump 4→5 (requires full audit of remaining cortexCluster consumers — dictionary, languageCortex, drug scheduler — to route through a main-cortex facade)
