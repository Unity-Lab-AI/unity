...
- [ ] **T39.f.3** — **sem_to_motor mean-cos STILL ~0.5 after the telemetry lie is removed — real overload root cause unresolved (Gee 2026-04-23).** With the CPU-shadow telemetry lie exposed, the authoritative signal is the sep-probe cosine which reads motor via live propagate. That stat persistently reports `mean-cos = 0.5-0.6` across every phase even with Oja + anti-Hebbian + WTA + lateral inhibition all shipped. Candidate root causes that need operator Part 2 data to distinguish: (a) motor cross-projection fan-in is too sparse for K-grade vocabulary diversity — with ~5 nonzeros per motor neuron via current `CROSS_DENSITY_CAP=0.005` + `crossTargetFanout=30` settings (T37 rebalance), 46 different opposite-pairs can't carve separable basins; (b) GloVe embeddings for K-grade content words cluster in a small subspace (cosine similarity high by construction) so sem-patterns for different pairs ARE genuinely similar and motor discrimination isn't the bottleneck; (c) GPU Oja is running with the wrong lr/reps combo and weight accumulation stalls at a fixed point before basins diverge. **OPEN** — next operator run with T39.f.1 fix (destructive normalize removed) will give cleaner sep-probe numbers. If mean-cos drops to ≤0.3 on the next run, the normalize was the bottleneck and T39.b closure gate closes. If mean-cos stays ~0.5, the fan-in / embedding-geometry hypothesis wins and the fix is a motor-region-specific density bump + per-cell embedding-whitening — a deeper lift that needs a dedicated session.
- [ ] **T39.f.4** — **K-STUDENT returned empty string even after the readiness gate passed — `'mcaa'` for 'a' cue, `'∅'` for b/c/d/e; system froze after SCI-K vocab audit (Gee 2026-04-23).** Operator saw readiness cue 1 emit `'mcaa'` (unicode junk — "WHY IS IT EVEN HAVE ACCESS TO UNICODE" — ) then cues 2-5 emit '?' or '.' or ∅. Four distinct problems compounded: letter inventory appears to have grown beyond a-z (unicode pollution at `encodeLetter` → needs lock); readiness probe threshold was weak (1/5 vs target 3/5 but still reports `canTalkAtAll=false` correctly here); the full K-STUDENT battery above ran on a brain that couldn't even recite the alphabet. Plus the system FROZE after `SCI-K vocab-coverage 0%` warning — suggesting the pre-gate enrichment path tried to bootstrap 358 untaught words in sequence and blocked the event loop. **OPEN** — T39.f.1 fix removes one destructive layer; next run will surface whether the authoritative sep-probe cosine drops, and the emission diagnostics added in T39.e.2 will pinpoint which subsystem is still broken. The alphabet-inventory lock + pre-gate chunking + per-cell vocab bootstrap are all queued for a dedicated session once T39.f.1 is validated on operator side.
