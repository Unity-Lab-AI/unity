...
- ~~**T15.B** — Architecture design~~ — **SHIPPED Session 114.19ab** (`docs/T15-architecture.md`)
- ~~**T15.C** — Implementation tasks~~ — **ALL 12 deliverables SHIPPED Session 114.19ac** across 7 atomic commits: `e7bd8f2` COMBOS + combo-aware contributions/speech + riskFlags + cravings + 13-axis speech + persistence v2; `3de2c2b` decide() + drug-rejections + nicotine/caffeine detector; `6764480` PATTERNS engine; `8f4c1a2` sensory-olfactory + drug-sensory-triggers; `838c9c3` main tick-loop _driveDrugScheduler + text-path decide/ingest/rejection routing; dashboard renderDrugPanel + language-cortex 4-axis (warmth/profoundBias/interruptionBias/confessionalBias) post-processors + LAW-6 firstUse ledger + markTrauma + lifeInfoLedger accessor + persistence extension. T15.C closed.
- ~~**T15.D** — Manual V1-V11 verification~~ — **REMOVED from the backlog entirely** per Gee 2026-04-18: *"T15D is NOT going to be done and can be removed"*. Not deferred; deleted. The scheduler's code paths are exercised via normal operation during pre-K + K Life cells (caffeine age-8 plus persona exclusions); there is no separate verification phase.
...
**T18.10 STATUS UPDATE 2026-04-19 (Session 114.19ai):** SHIPPED but INCOMPLETE. Gee ran Part 2 K after T18.10 landed and the PC-reset cascade fired again during ELA-K startup. Root-cause re-investigation found T18.10 only patched the validation-FAILURE branch — the success-path `this._sparseMatrices[name] = entry` overwrite still orphaned the old entry's buffers on ANY re-upload trigger. Plus two independent re-upload triggers were identified: stale compute.html tab contention from server restarts, and flat 3 s reconnect with no backoff. **Completed by T18.11 below.**

---

#### T18.11 — "els K START evently like ran endless ly and crashed: BRain states at loss of internet connection" (Gee 2026-04-19)
...
T18.10 shipped destroy-on-failure for `uploadSparseMatrix` + `_beginSparseUpload` validation failure paths, but missed the SUCCESS-PATH leak: both functions assign `this._sparseMatrices[name] = entry` without destroying any prior entry's buffers. On ANY re-upload with the same name, 3 buffers (cluster-bound) or 6 buffers (standalone: + preSpikes + postCurrents + postSpikes) orphan at 100-600 MB each.

Plus two re-upload triggers found that T18.10 never accounted for:

1. **Stale `compute.html` tab from prior server run.** `_spawnGpuClient` auto-opens a new browser tab on every server boot regardless of whether a prior tab is still alive. The prior tab's WS auto-reconnects to the new server within 3 s and keeps its GPU buffers allocated. A fresh tab ALSO launches and runs its own biological-scale init. Two compute.html instances × ~8 GB each = 16 GB OOM on a 4070 Ti SUPER in one try.
...
Either trigger crosses the success-path leak and stacks multi-GB of orphaned buffers per cycle → VRAM exhaustion → `device.lost` → Windows TDR → NDIS/WinSock cascade → whole PC loses internet → hard reset. Same symptom chain T18.10 described, but via a path T18.10 didn't close.

- [x] **T18.11.a — `uploadSparseMatrix` + `_beginSparseUpload` destroy old entry before overwrite.** New `_destroySparseEntryBuffers(entry)` helper handles all 3-6 buffer variants safely (each `.destroy()` wrapped in try/catch so double-free on a dead device is non-fatal, entry=undefined is a no-op). Both upload sites call it as the first operation after the `_available` guard so the leak is impossible on any re-upload path — past, present, or future. **SHIPPED** — `js/brain/gpu-compute.js` (+40 lines).
- [x] **T18.11.b — `_spawnGpuClient` skip when GPU client already connected.** Guard clause at the top of `_spawnGpuClient`: `if (brain && brain._gpuClient && brain._gpuClient.readyState === 1) return;`. Spawn delay bumped from 500 ms to 3500 ms to match the compute.html first-retry reconnect window (3 s + 500 ms scheduling margin). Pre-existing compute.html tabs from prior server runs reconnect during that window, so the guard sees the active client and skips the fresh-tab launch. Server logs `[Server] GPU compute client already connected from prior session — skipping auto-launch`. **SHIPPED** — `server/brain-server.js` (+29 lines).
- [x] **T18.11.c — `compute.html` exponential-backoff reconnect.** Module-level `_reconnectAttempt` counter. `ws.onclose` computes `delaySec = Math.min(60, 3 * Math.pow(2, _reconnectAttempt))` → 3 s, 6 s, 12 s, 24 s, 48 s, 60 s cap. Counter resets to 0 on `ws.onopen` so a brief hiccup after a stable connection doesn't carry forward a stale exponent. Status bar shows attempt number. **SHIPPED** — `compute.html` (+27 lines).
- [x] **T18.11.d — Docs sweep (atomic per LAW "Docs before push, no patches").** `docs/NOW.md` full rewrite for session 114.19ai. `docs/TODO.md` T18.10 status update + T18.11 entry (this block). `docs/FINALIZED.md` session 114.19ai entry prepended. **SHIPPED** — all docs in this commit.

**T18.11 closure gate:** Gee-verification on next Part 2 run. Success criteria: (a) no PC-reset, (b) `[Server] GPU compute client already connected from prior session — skipping auto-launch` fires when a stale tab is present, (c) `compute.html` status bar shows exponential delays on any transient disconnect, (d) if a re-upload does fire both the T18.10 destroy-on-failure AND the T18.11 destroy-old-entry logs are visible (belt + suspenders). Claude cannot close — Gee-verification only.
...
#### T18.13 — Pre-K skip fix + DREAM_MAX_GRADE stop gate + 5-second teach heartbeat (Gee 2026-04-19)

**Gee's verbatim 2026-04-19:**

> *"its just running away: 183compute.html:163 [GPU Compute] binary frame received size=0.0MB, first4=SPRS"*
>
> *"i closed it before it killed my ethernet card"* — server log tail showed `[Curriculum] ela/kindergarten START` with NO `ela/pre-K START` before it
>
> *"push to syllabus if all is ficxed now and its actually going to show ela progress and not get hung on the shit"*

Gee's first T18.12 Part 2 run exposed three bugs: (a) Pre-K skipped entirely — my 5 new Pre-K runners never fired; (b) curriculum walked toward PhD by default (Pre-K + K ONLY LAW violation); (c) teach heartbeat was `_wordIdx % 200 === 0` which never triggered on typical 180-word K emission lists — Gee watched the terminal in silence for minutes with no progress indicator. T18.13 fixes all three atomic.

- [x] **T18.13.a — Pre-K skip fix.** `runAllSubjects` had hardcoded `for (let i = 1; i < GRADE_ORDER.length; i++)` at ~line 2186 — skipped GRADE_ORDER[0]='pre-K' for every subject regardless of passedCells state. Fix: start at `i=0`, T18.12.c resume-skip handles already-passed cells. `runFullSubjectCurriculum` had the same issue via `const startIdx = Math.max(0, GRADE_ORDER.indexOf(current) + 1)` (default `current = 'pre-K'` → startIdx=1) AND an explicit `if (grade === 'pre-K') continue;`. Both removed. New `_computeResumeStartIdx(subject)` helper consults `cluster.passedCells` as authoritative source: fresh brain returns 0 (pre-K); resumed brain returns `highest-passed-idx + 1`. **SHIPPED** — `js/brain/curriculum.js`.
- [x] **T18.13.b — DREAM_MAX_GRADE stop gate.** Curriculum now caps at `'kindergarten'` by default per Pre-K + K ONLY LAW (Gee 2026-04-18). New `_resolveMaxGradeIdx()` helper reads `process.env.DREAM_MAX_GRADE`; `'phd'` unsets the cap. Both `runAllSubjects` + `runFullSubjectCurriculum` break with `⏹ T18.13 stop — reached grade cap 'kindergarten'` when loop index exceeds cap. Boot-time log: `[Curriculum] T18.13 grade cap = 'kindergarten' (set DREAM_MAX_GRADE env to change...)`. Updated `runCompleteCurriculum` top-line log from `walking all 6 subjects K→PhD` to `walking all 6 subjects pre-K onward (cap via DREAM_MAX_GRADE; default 'kindergarten' per Pre-K + K ONLY LAW)`. **SHIPPED** — `js/brain/curriculum.js`.
- [x] **T18.13.c — 5-second teach heartbeat.** `_teachWordEmission` + `_teachPhonemeBlending` both had `if (_wordIdx % 200 === 0)` progress logs that never fired on ~180-word lists. Added time-based heartbeat inside the word loop: every 5 s of wall-clock emit `[Curriculum] ⏱ _teachWordEmission heartbeat — rep N/reps, word M/total, elapsed Xs, ~Y words/s`. Gee can now watch the terminal and distinguish "slow but advancing" (heartbeats ticking at ~0.5-2 words/s at biological scale) from "genuinely hung" (heartbeat stops for > 30 s). **SHIPPED** — `js/brain/curriculum.js`.

**T18.13 closure gate:** Gee-verification on next Part 2 run. Success criteria: (a) server log shows `ela/pre-K START` + `sci/pre-K START` + `math/pre-K START` + `social/pre-K START` + `art/pre-K START` BEFORE any `ela/kindergarten START`; (b) heartbeat lines fire every 5 s during teach; (c) curriculum STOPS at kindergarten with `⏹ T18.13 stop` log line, not rolling into G1; (d) no PC reset / no ethernet cascade (T18.11 protection path untouched by T18.13). Claude cannot close — Gee-verification only.

---

#### T18.14 — ELA-K ethernet cascade (Gee 2026-04-19) — two GPU-side leaks T18.10/11 didn't catch
...
And the server terminal tail he pasted showing compute.html `Disconnected — reconnecting in 6s... (attempt 2)` followed by a flood of `[GPU Compute] binary frame received size=16.0MB, first4=SPRS` frames immediately after `[Curriculum] ela/kindergarten START`, culminating in *"That is everything!!!! right before it freezes and kills the connection ofe my PC to the internet"*.

**Diagnosis — two independent GPU-side leak paths stacking on top of each other, both firing during ELA-K teach, both causing the exact PC-reset/ethernet-death cascade T18.10 / T18.11 tried to close but missed:**

**Leak 1 — `hebbianSparse` paramsBuf leak at `js/brain/gpu-compute.js:1609`.** The function allocates a 32-byte uniform buffer at line 1590 (`paramsBuf = this._createBuffer(...)`) every single call, then at line 1609 the comment literally says *"paramsBuf destroyed lazily by device GC"* — which is a LIE. WebGPU does not garbage collect buffers. They persist until explicit `.destroy()` or device destruction. At ELA-K teach velocity through T18.8 batched-Hebbian dispatch, each teach pass fires `~180 words × 12 reps × 14 cross-projections = ~30,000 hebbianSparse calls`. Each call orphans a WebGPU buffer handle. Driver allocation tables are finite — NVIDIA drivers typically cap at ~65K concurrent buffer allocations, Microsoft Windows imposes its own per-process limits on top. After ~30K leaked handles per teach pass, the driver exhausts → `device.lost` fires → Windows TDR attempts GPU driver reset → NDIS/WinSock cascade via shared driver-stack resources → whole PC loses internet → Gee's only recovery is a physical PC reset. Compare to the correctly-written `propagateSparse` at line 1549 which calls `paramsBuf.destroy()` explicitly after submit.

**Leak 2 — `uploadCluster` LIF buffer orphan on WS reconnect at `js/brain/gpu-compute.js:655`.** The server's `ws.on('close')` at `server/brain-server.js:4566` resets `brain._gpuInitialized = {}` so the tick loop re-sends `gpu_init` messages for all 7 clusters on reconnect. compute.html's `gpu_init` handler at line 398 calls `gpu.uploadCluster(clusterName, size, ...)` which allocates fresh buffers (voltages, spikes, currents, params, regionGates) and at line 655 assigns `this._buffers[name] = buffers` — WITHOUT destroying the old entry. At biological scale: cortex (107M × 16 bytes = ~1.7 GB), cerebellum (143M × 16 bytes = ~2.3 GB), hippocampus (~700 MB), amygdala (~460 MB), basalGanglia (~460 MB), hypothalamus (~340 MB), mystery (~340 MB) → **~6.3 GB of VRAM orphaned per WS reconnect cycle**. On a 16 GB RTX 4070 Ti SUPER, even ONE WS reconnect during ELA-K teach puts VRAM over budget. Combined with the existing ~6 GB of sparse matrices + the Leak 1 handle accumulation → `device.lost` → TDR → NDIS cascade → internet dies.

**Leak 3 (secondary, cost-avoidance rather than VRAM) — compute.html `gpu_init` handler re-allocates even when cluster already initialized at `compute.html:384-413`.** When the same compute.html tab reconnects after a transient WS hiccup, the GPU context is still alive and `clusterState[clusterName].initialized === true`, but the handler blindly calls `gpu.uploadCluster(...)` anyway. With Leak 2 fixed this at least no longer leaks — but the ~6.3 GB of allocation work is still wasteful + delays the main brain coming back online, widening the window for another WS hiccup → reconnect storm.

- [x] **T18.14.a — `hebbianSparse` paramsBuf destroy after submit.** Single-line addition at `js/brain/gpu-compute.js:1609`: `paramsBuf.destroy();` after `device.queue.submit([encoder.finish()])`. WebGPU spec says buffer destruction after submit is legal — the GPU can still use the buffer's contents from the submitted command buffer until the work completes; destroy() releases the resource handle afterward. Matches the correct pattern already in `propagateSparse` at line 1549. Kills the 30K-per-teach handle leak dead. **SHIPPED** — `js/brain/gpu-compute.js`.
- [x] **T18.14.b — `uploadCluster` destroys old cluster buffers before overwriting `_buffers[name]`.** New helper method `_destroyClusterBuffers(bufs)` on `GPUCompute` iterates every possible buffer field on a cluster buffers object: `voltages`, `spikes`, `currents`, `params`, `regionGates`, `synValues`, `synColIdx`, `synRowPtr`, `voltSumBuf`, `spikeCountBuf`. Each `.destroy()` wrapped in try/catch so a double-free on a dead device is non-fatal; `bufs=undefined` is a no-op. `uploadCluster` calls it as the first operation (before any new allocations) on the existing entry in `this._buffers[name]`. Belt-and-suspenders discipline matching T18.11's `_destroySparseEntryBuffers` — the leak is now impossible on any re-init path, existing or future. **SHIPPED** — `js/brain/gpu-compute.js`.
- [x] **T18.14.c — compute.html `gpu_init` skip re-upload when cluster already initialized.** Guard clause at the top of the `gpu_init` handler checks `if (clusterState[clusterName] && clusterState[clusterName].initialized && clusterState[clusterName].size === size)` → ACKs immediately via `gpu_init_ack` without calling `gpu.uploadCluster`. Preserves cluster state across transient WS disconnects (same-tab reconnect); avoids the ~6.3 GB of wasted allocation work + closes the reconnect-storm widening. Server's `ws.on('close')` still resets `_gpuInitialized` for tick-loop safety, but compute.html's short-circuit makes that a cheap JSON round-trip instead of a 6.3 GB VRAM replay. If size changes (legitimate reason to re-init, e.g. server restart with different config): falls through to the existing uploadCluster path, which now also has T18.14.b's destroy-old guard. **SHIPPED** — `compute.html`.

**T18.14 closure gate:** Gee-verification on next Part 2 run. Success criteria: (a) no PC reset / no ethernet cascade during ELA-K teach; (b) if a WS disconnect does fire, the reconnect completes within the exponential-backoff window + compute.html status bar shows `Skipped re-init for cluster X (already initialized)` log lines (new T18.14.c signal); (c) heartbeat lines from T18.13.c continue firing through any transient disconnect; (d) curriculum progresses past ELA-K first course (`_teachWordEmission` on K vocab list) into subsequent courses. Claude cannot close — Gee-verification only.
...
- [x] **T18.34.b — `_teachPhonemeBlending ~3 words/s` velocity.** **RESOLVED by accepting baseline Session 114.19az.** The operator's Part 2 retest actually showed `_teachWordEmission heartbeat — ~19.6 words/s` at end — meaning the actual sustained velocity is already 6-7× higher than the feared 3 w/s number. The earlier 3 w/s came from a different session's curriculum state. Baseline 19-25 w/s is acceptable — full K teach completes in ~10-15 minutes. Closing T18.34.b without further changes. Two attempts reverted this session (worker-pool routing caused 20× regression — see previous entry). If teach velocity drops again in a future session, options remain: batch whitelist Hebbian across letters before pool dispatch, move whitelist back to GPU-only once motor activation validated.
...
- [x] **T18.40.c — Rep 10 of `_teachPhonemeBlending` at 2-3 words/s.** **SHIPPED Session 114.19az follow-on.** Added final-rep sampling mode to `cluster._crossRegionHebbian`: new flag `_teachFinalRepSampleEveryN` controls how often CPU whitelist Hebbian runs on probe-critical projections. Teach loops set it to 5 on the final rep (runs every 5th call) and 0 on intermediate reps (my earlier `_teachIntermediateRep` still handles those). GPU fire-and-forget runs every call regardless, so GPU weights are fully current; CPU arrays see 20 % of the final-rep updates — representative enough for probes given prior 9 reps left CPU stale anyway. Expected ~5× final-rep speedup (2-3 w/s → ~15 w/s sustained). Wired in both `_teachPhonemeBlending` and `_teachWordEmission` — flag set at rep-entry and cleared at phase DONE.

- [x] **T18.40.d — Mid-phase checkpoint saves so Savestart.bat resumes finer-grained.** **SHIPPED Session 114.19az follow-on.** `_phaseDone(name)` now records the phase in `cluster.passedPhases = ['ela/kindergarten:_teachLetterCaseBinding', ...]` and fires `this._saveCheckpoint('ela/kindergarten:phase:<name>')` after each DONE banner. The existing T18.35.b JSON save path persists `passedPhases` alongside `passedCells` automatically (they live on the cortex cluster which is already in the cortexState save block). If the brain crashes mid-cell, weights trained up to the last DONE phase persist on disk — Savestart resumes with weights intact. Full mid-cell resume logic (skip already-done phases on runner re-entry) is a follow-on; for now the weight durability alone is a large win (no more losing 10+ minutes of prior-phase training on a crash in phase 8 of 11). Added alongside the same banner fix so both T18.40.d + T18.40.e land together.

- [x] **T18.40.e — Phase banners for the six "invisible" teach phases.** **SHIPPED Session 114.19az follow-on.** All 6 previously-silent teach phases (`_teachPluralTransform`, `_teachQuestionWordCategories`, `_teachEndPunctuation`, `_teachCapitalization`, `_teachStoryComprehension`, `_teachCausalChains`) now wrapped with `_phaseTick(name)` + `_phaseDone(name)` so they emit `🧩 ELA-K Phase START — <name>` and `✓ ELA-K Phase DONE — <name> in Xs` banners like the other phases. Also added banners to `_teachPhonemeBlending` + `_teachWordEmission` so every teach phase shows START/DONE. Combined with T18.40.d mid-phase save, every phase completion now produces a visible banner AND a disk-persisted checkpoint.

- [x] **T18.40.f — Kindergarten vocabulary needs life-experience words before Life-K events.** **SHIPPED Session 114.19az follow-on.** New `K_LIFE_EXPERIENCES` word-array added to `_runElaKReal` with ~80 life-event comprehension words across 6 sub-groups: memory/narration (remember / forget / happened / story / first-time), family milestones (birth / wedding / funeral / visit / vacation / graduate), social-emotional events (fight / forgive / apologize / secret / promise / lie / truth / fair / mistake), health-care (doctor / hospital / medicine / shot / bandaid / bruise / stitches / glasses), caregiver roles (babysitter / guardian / stepmom / adopted / foster), places of life events (funeral-home / court / jail / daycare / clinic), event connectors (ago / once / suddenly / finally / never / always / someday). Merged into `allEmissionWords` via `...K_LIFE_EXPERIENCES` spread + category name added to the vocab-count log. Per-grade Life Vocabulary Prerequisites section added to `docs/TODO-full-syllabus.md` — covers Pre-K + K in scope now (shipped) with the binding rule: *before any grade's Life-track teaches an event, the academic tracks at that grade must have trained the comprehension vocabulary*. Post-K reference examples included (G7 first joint vocab, G8 first drink vocab, G9 first line vocab, etc.) — DEFERRED per PRE-K + K ONLY LAW but the rule binds when they unlock.
...
#### T18.37 — log-file redirect + tail window so invisible-terminal doesn't blind the operator (Gee 2026-04-20)

**Gee's verbatim 2026-04-20 (post-T18.36 push):**

> *"its not working the start.bat window console that suppose to open is like erororing and is not properly displaying fully and is like invisible but the brower windows are opening it just the terminal console of all the heartbereat and brain information is invisiblke, should i resaart my computer?"*

Diagnosis: brain IS running fine (node PID 13508 at 10 GB RSS listening on :7525 with ESTABLISHED connection — server healthy, browser can reach it, compute.html loaded). The `start.bat` cmd window itself can't render because Windows Terminal / conhost has a GPU-rendering / DWM glitch on Gee's machine — 7 conhost.exe zombies + 1 WindowsTerminal.exe piled up from prior sessions. **PC restart clears that.** But T18.37 ensures future invisible-terminal events don't blind Gee from heartbeat + brain info.

- [x] **T18.37.a — Redirect brain-server stdout/stderr to `server\server.log`.** `start.bat` and `Savestart.bat` both now spawn node via `start /b "" cmd /c "node ... > server.log 2>&1"`. Log file lives at `server\server.log`, always readable from any terminal via `Get-Content` / `type` / `tail`, survives the launcher terminal going invisible. **SHIPPED** — `start.bat` + `Savestart.bat`.
- [x] **T18.37.b — Separate PowerShell tail window via `Get-Content -Wait`.** Second `start "Unity Brain Log Tail" powershell -NoExit -Command "Get-Content -Path server.log -Wait -Tail 200"` spawns a fresh PowerShell process whose window rendering is independent of the launcher terminal. Even if the launcher went invisible again, the tail window paints normally — different process, different conhost instance. **SHIPPED**.
