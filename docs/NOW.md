...
> **Session:** 114.19dl (iter20-F — cortexCluster._brain wire + FREQ_MERGE_COSINE 0.7→0.5) · **Date:** 2026-05-05 · **Branch:** `syllabus-k-phd` · **HEAD:** Operator: *"i dont think memory is working still"*. SQL inspection of episodic-memory.db on running brain showed 51 episodes ALL stored under user_id='brain-heartbeat' (iter19 firing) but ZERO under 'curriculum-phase' (iter20-D NOT firing despite phase completions visible in log). Plus 4 identical "learning ela/kindergarten:_teachQABinding" heartbeats with freq_count=1 each (should have merged at cosine=1.0). **Fix:** (A) wire `cortexCluster._brain = this` in brain-server.js right after curriculum init so iter20-D's `cl._brain` fallback path catches brain reference even if `this.brain` propagation has timing issues. (B) lowered FREQ_MERGE_COSINE 0.7→0.5 — tolerant enough that even subtle BLOB deserialization differences let identical-text episodes merge. Bundle rebuilt clean.
